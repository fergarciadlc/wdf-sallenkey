...
def calculate_frequency_response(filter_instance: Union[RCLowPass, RCHighPass, RCBandPass1st, RCBandPass2nd],
                               sample_rate: float,
                               fft_order: int = 14) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    Tuple[np.ndarray, np.ndarray]
        Arrays containing frequencies and magnitudes (in dB)
    """
    ...
    magnitudes = 20 * np.log10(np.abs(spectrum))
...
def write_csv(filename: Path, frequencies: np.ndarray, magnitudes: np.ndarray) -> None:
    """
...
def main():
    ...
    for order in [1, 2]:
        ...
        frequencies, magnitudes = calculate_frequency_response(filter_instance, SAMPLE_RATE, FFT_ORDER)
        filename = generate_filename("LowPass", order, CUTOFF_FREQ)
        write_csv(output_dir / filename, frequencies, magnitudes)
